...
// lijing 增加
	std::string channelName = connection->getInputChannelName();
	if (eventEntry->type == EventEntry::TYPE_MOTION) {
		MotionEntry* m = static_cast<MotionEntry *>(eventEntry);

		if ((m->source & AINPUT_SOURCE_MOUSE) == AINPUT_SOURCE_MOUSE
				&& channelName.find("PointerEventDispatcher") == std::string::npos
				&& channelName.find("com.android.launcher3") == std::string::npos
				&& channelName.find("edge-swipe") == std::string::npos
				&& channelName.find("swipe-up") == std::string::npos
				) {
			// lijing: 对于应用忽略三指操作
			if (m->pointerCount >= 3) {
				return;
